...
public class ReporteGrupalProcesoConsolidado extends BaseReport {
	...
	public ByteArrayOutputStream getReport(Map<String, String[]> params, String localPath, ServletContext servlet,
		...
		try {
			...
			String tipo = params.get("p2")[0];
			paramRpt.put("par_id_usuario", idUsuario);
			paramRpt.put("SUBREPORT_DIR", localPath + File.separator + "reporte" + File.separator);
			if (tipo.equals("Cualitativo")) {
				if (isexcel) {
					nombreReporte = "rptProcesoCualitativoExcel.jasper";
				} else {
					nombreReporte = "rptProcesoCualitativoPdf.jasper";
				}
			} else {
				if (isexcel) {
					nombreReporte = "rptProcesoConsolidadoExcel.jasper";
				} else {
					nombreReporte = "rptProcesoConsolidadoPdf.jasper";
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
