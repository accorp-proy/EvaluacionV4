...
public class ReporteGrupalEvaluacionConsolidado extends BaseReport {
	...
	public ByteArrayOutputStream getReport(Map<String, String[]> params, String localPath, ServletContext servlet,
		...
		try {
			anioC = params.get("p3")[0];
			if (anioC.equals("")) {
				Calendar calendar = Calendar.getInstance();
				anioC = String.valueOf(calendar.get(Calendar.YEAR));
			}
			Long idUsuario = Long.parseLong(params.get("p1")[0]);
			Long idZona = params.get("p2")[0].equals("") ? 0L : Long.parseLong(params.get("p2")[0]);
			paramRpt.put("par_id_usuario", idUsuario);
			paramRpt.put("par_id_zona", idZona);
			paramRpt.put("par_anio", anioC);
			paramRpt.put("SUBREPORT_DIR", localPath + File.separator + "reporte" + File.separator);
			if (isexcel) {
				nombreReporte = "rptEvaluacionConsolidadoExcel.jasper";
			} else {
				nombreReporte = "rptEvaluacionConsolidadoExcel.jasper";
			}
		} catch (Exception e) {
